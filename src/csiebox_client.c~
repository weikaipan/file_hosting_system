...
// For MP4 handle busy
static csiebox_protocol_header wait(int conn_fd, void* message, size_t len);
...
static csiebox_protocol_header wait(int conn_fd, void* message, size_t len) {
  ...
  while (busy) {
    fprintf(stderr, "wait.\n");
    if (!send_message(conn_fd, message, len)) {
      fprintf(stderr, "send fail!\n");
    } else {
      fprintf(stderr, "send request in busy handler\n");
    }
    ...
    if (header.res.status == CSIEBOX_PROTOCOL_STATUS_BUSY){
      printf("Server busy\n");
      sleep(3);  // wait for 3s then retry
    } else {
       busy = 0;
    }
  }
...
static int login(csiebox_client* client) {
  ...
  memset(&header, 0, sizeof(header));
  //  MP4
  header = wait(client->conn_fd, &req, sizeof(req));
...
static int prepare_and_sync(csiebox_client* client) {
 ...
 /* 
  ...
  free(longest_path);
 */ 
  csiebox_protocol_header header_req, rev;
  memset(&header_req, 0, sizeof(header_req));
  header_req.req.magic = CSIEBOX_PROTOCOL_MAGIC_REQ;
  header_req.req.op = CSIEBOX_PROTOCOL_OP_SYNC_END;
  header_req.req.client_id = client->client_id;
  // receive busy mesage until OK
  fprintf(stderr, "Send End\n");
  rev = wait(client->conn_fd, &header_req, sizeof(header_req));
  if (rev.res.status == CSIEBOX_PROTOCOL_STATUS_OK)
    fprintf(stderr, "Sync end ok\n");
  chdir(cwd);
  free(cwd);
  return 1;
}
...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path) {
  ...
  int fd = open(path, O_CREAT | O_WRONLY | O_TRUNC, REG_S_FLAG);
  ...
  */
  ...
  // MP4 only have to be care about meta
  //send_message(client->conn_fd, &meta, sizeof(meta));
  csiebox_protocol_header rev; 
  memset(&rev, 0, sizeof(rev));
  fprintf(stderr, "Send meta\n");
  rev = wait(client->conn_fd, &meta, sizeof(meta));
  if (rev.res.status == CSIEBOX_PROTOCOL_STATUS_MORE)
    fprintf(stderr, "Request sent\n");
  // if server busy, client will receive busy protocol
...
static void sync_file_data(
  ...
  if ((stat.st_mode & S_IFMT) == S_IFDIR) {
    ...
    if (fd < 0) {
      ...
      size_t readlen;
      while ((readlen = read(fd, buf, FILE_SIZE)) > 0) {
...
static void rm_file(csiebox_client* client, char* path, int is_dir) {
  ...
  // MP4 add here only have to be care about rm
  //send_message(client->conn_fd, &rm, sizeof(rm));
  csiebox_protocol_header rev; 
  memset(&rev, 0, sizeof(rev));  
  fprintf(stderr, "Send rm\n"); 
  rev = wait(client->conn_fd, &rm, sizeof(rm));
  if (rev.res.status == CSIEBOX_PROTOCOL_STATUS_MORE)
    fprintf(stderr, "Request sent\n");
